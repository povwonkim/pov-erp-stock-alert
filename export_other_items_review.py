#!/usr/bin/env python3
"""브랜드="OTHER"(여러 실제 브랜드가 섞인 통 분류)에 속한 품목을 품목 단위로 검토할 수 있게
엑셀로 뽑는다(2026-08-07 사용자 요청 — "각각의 품목을 알아야돼서 품목명이랑 현재
재고수량을 같이 보여주면 내가 표시해줄게").

RAW_품목마스터(이미 등록된 것)와 one_time_seed_2026-07-30/full_item_registry_2026-08-07.json
(이카운트 품목등록 전체 조회 — 아직 등록 안 된 것 포함) 양쪽에서 브랜드="OTHER"인 품목코드를
모으고, RAW_재고현황(오프라인 4개 창고 합계)에서 현재 재고수량을 붙인다.

사용법 (서버에서):
  .venv/bin/python export_other_items_review.py --spreadsheet-id <ID> --out other_items_review.xlsx
"""
...
def main() -> int:
    ...
    ap.add_argument("--out", default="other_items_review.xlsx")
    args = ap.parse_args()
    ...
    # 품목코드 -> 품목명, 브랜드="OTHER"인 것만.
    items: dict[str, str] = {}

    master_rows = read_tab_rows(service, args.spreadsheet_id, "RAW_품목마스터")
    for r in master_rows:
        if (r.get("브랜드") or "").strip() == "OTHER":
            items[r["품목코드"]] = r.get("품목명", "")

    registry = json.loads(REGISTRY_PATH.read_text(encoding="utf-8"))
    for item in registry:
        if item.get("브랜드") == "OTHER" and item["품목코드"] not in items:
            items[item["품목코드"]] = item.get("품목명", "")

    print(f"[export] OTHER 브랜드 품목 {len(items)}건 발견 (품목마스터+전체등록 합산)")
