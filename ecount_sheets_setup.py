...
TABS: dict[str, dict] = {
    ...
    # 재고 1~2개인데 입고·판매 둘 다 60일 이상 없는 품목 — 실재고가 아니라 진열/샘플용일 가능성.
    # 가용재고로 잘못 읽힐 수 있어 사람이 이카운트에서 직접 확인하도록 별도로 뽑는다(2026-07-28).
    # 이 시스템이 직접 쌓은 일별재고이력이 60일 이상 있어야 정확해진다 — 도입 초기엔 "미판매(일)/
    # 미입고(일)"을 판단할 근거 자체가 없어서 거의 비어 있는 게 정상(오탐 방지를 위해 근거 없으면
    # 아예 후보에서 제외, 빈 리스트=이상 없음이 아니라 아직 판단 불가일 수 있음에 유의).
    "샘플의심재고": {
        "note": [
            "📋 재고 1~2개 & 입고·판매 둘 다 60일 이상 없음 — 진열/샘플용일 가능성, 실재고 아닐 수 있음",
            "이카운트에서 실제 샘플/전시용인지 확인 후 처리 (가용재고로 잘못 잡히지 않게)",
            "정렬: 미판매(일) 내림차순 · 🕒 마지막 갱신: 매일 자동 (60일치 이력 쌓이기 전엔 대부분 공란)",
        ],
        "headers": [
            "브랜드", "품목코드", "품목명", "조달유형",
