...
QTY_COLS = {"재고수량", "재고", "총재고", "전일재고", "입고", "출고",
            ...
            # 기타창고_재고현황의 개별 창고 컬럼(2026-08-07) — 값이 있는 칸만 숫자로 채워짐.
            "29CM", "시시호시-수원점", "신세계 강남-피숀", "온라인", "올리브영", "올리브영(성수N)",
            "올리브영(홍대)", "카카오 선물하기(단품 재고)", "카카오 선물하기(세트 재고)", "DDP디자인스토어",
            "HQ_office", "MXN(29cm)", "MXN(올리브영)", "MXN(카카오)", "orer.archive", "OTS", "OY",
            "POP-UP", "POV"}
...
# 넣는다(단, RAW_일별재고이력의 "날짜"는 매 행이 실제로 다른 값이라 예외 — 그대로 컬럼 유지).
# 3층 구조 (카페24 온라인 대조 시스템과 동일한 패턴을 오프라인에 적용, DOI 기반 우선순위 포함):
#   1층 RAW(원본 수집) → 2층 RAW_일별재고이력(DOI·상태판정 엔진) → 3층 결과물(용도별 뷰)
TABS: dict[str, dict] = {
    ...
    # 2층 RAW_일별재고이력 — 3층의 모든 결과물이 여기서 계산돼 나오는 엔진. 사람이 매일 볼 필요는
    ...
    # 위험/주의 임계값: 자체제작 DOI≤30일/30~44일 · 국내사입 DOI≤7일/7~14일 · 국내위탁 DOI≤7일/7~14일
    #                · 해외수입 DOI≤21일/21~35일
    "RAW_일별재고이력": {
        "note": [
    ...
    "디자인팀_발주필요": {
        "note": [
            ...
            "국내사입/국내위탁/해외수입 브랜드는 여기 안 뜸 — 전체 재고는 관리팀_전체재고 참고",
    ...
    # 이 시스템이 직접 쌓은 RAW_일별재고이력이 90일 이상 있어야 정확해진다 — 도입 초기엔 "미판매(일)/
    ...
    # 더는 POV가 취급하지 않는 옛(전신) 브랜드인데 재고가 남아있는 것 — 몇 년째 처리 안 된
    # 진짜 방치 자산(2026-08-07 사용자 확정). 관리팀_전체재고 등 일반 운영 탭에는 안 뜨고
    # 여기에만 별도로 모아서, 폐기/땡처리/재확인 등 최종 결정을 내리는 용도.
    # OLD_BRANDS(ecount_item_master.py)에 등록된 브랜드만 대상 — 재고 0이면 여기도 안 뜸.
    "구브랜드_방치재고": {
        "note": [
            "📋 더는 취급 안 하는 옛 브랜드인데 재고가 남아있는 것 — 몇 년째 방치된 자산, 반드시 처리 필요",
            "일반 운영 탭(관리팀_전체재고 등)엔 안 뜸 — 여기서만 확인 가능. 폐기/땡처리/재확인 중 결정",
            "정렬: 재고금액 내림차순 · 🕒 마지막 갱신: 매일 자동",
        ],
        "headers": [
            "브랜드", "품목코드", "품목명", "조달유형",
    ...
    # 브랜드는 계속 취급하지만 그 브랜드 안의 특정 품목만 이카운트에서 "(단종)" 표시된 경우.
    # 예전엔 이런 품목을 발견 즉시 시스템 전체(RAW_일별재고이력 포함)에서 제외했는데, 그러면서
    # 재고가 남아있어도 어디에도 안 보이는 문제가 있었다(2026-08-08 확인 — 187건, 최대 74개
    # 재고). 구브랜드_방치재고와 같은 패턴: 이력은 정상 기록하고, 재고 남은 것만 여기 별도로
    # 뽑아서 처리 대상으로 계속 보이게 한다.
    "단종품목_재고현황": {
        "note": [
            "📋 브랜드는 계속 취급하지만 이 품목만 이카운트에서 단종 처리됐는데 재고가 남아있는 것",
            "일반 운영 탭(관리팀_전체재고 등)엔 안 뜸 — 여기서만 확인 가능. 폐기/땡처리/재확인 중 결정",
            "정렬: 재고금액 내림차순 · 🕒 마지막 갱신: 매일 자동",
        ],
        "headers": [
    ...
    # 이카운트에 등록된 창고 23개 전체 목록. 이 시스템이 자동 추적하는 건 OFFLINE_WAREHOUSES
    # 4곳뿐이고 나머지 19곳은 왜 있는 창고인지, 계속 써야 하는지, 정리해야 하는지가 불명확한
    # 상태(2026-08-07 확인) — 창고별 설명/처리방침은 담당자(수빈실장)가 직접 채우는 용도.
    "창고목록": {
        "note": [
            "📋 이카운트에 등록된 창고 23개 전체 — 이 중 4개(POINT OF VIEW 법인/THE HYUNDAI SEOUL/MXN/MXN(온라인))만 이 시스템이 매일 자동 추적",
            "나머지 19개는 무슨 창고인지, 계속 쓰는지, 정리해야 하는지 담당자가 설명/처리방침을 직접 작성",
            "🕒 수동 관리 — 자동 갱신 안 됨",
        ],
        "headers": ["창고코드", "창고명", "자동추적여부", "설명(담당자 작성)", "처리방침(담당자 작성)"],
    },
    # OFFLINE_WAREHOUSES 4곳을 제외한 나머지 19개 창고의 실제 재고 스냅샷(2026-08-07 사용자가
    # 이카운트에서 직접 뽑아준 엑셀 기반, 1회성 반영 — 이 19개 창고는 API로 자동 조회하지
    # 않으므로 매일 자동 갱신되지 않는다. 재고보유월수 컬럼이 살아있는 원본이라 그대로 살림).
    # "이게 진짜 방치재고" — 사용자 확정(2026-08-07): 메인 4개 창고 밖에 쌓여있는 게 진짜 문제.
    "기타창고_재고현황": {
        "note": [
            "📋 메인 4개 창고를 제외한 19개 창고의 재고 — 이게 진짜 방치재고(2026-08-07 사용자 확정)",
            "⚠️ API 자동조회 대상이 아니라 수동 갱신만 됨 — 스냅샷 날짜: 2026-08-08(엑셀 추출 시점)",
            "창고별 재고가 있는 칸만 숫자가 채워짐 · 정렬: 재고수량 내림차순",
        ],
        "headers": [
            "브랜드", "품목코드", "품명",
            "29CM", "시시호시-수원점", "신세계 강남-피숀", "온라인", "올리브영", "올리브영(성수N)",
            ...
            "HQ_office", "MXN(29cm)", "MXN(올리브영)", "MXN(카카오)", "orer.archive", "OTS", "OY",
            "POP-UP", "POV",
            "재고수량", "입고단가", "재고금액", "바코드", "재고보유월수", "조치", "메모",
        ],
    },
}
...
def _readme_content() -> list[tuple[str, list[str]]]:
    ...
    for r in [
        ...
        ["구브랜드_방치재고", "관리팀", "월 1회", "메모만", "더는 안 취급하는 옛 브랜드인데 재고 남은 것 — 폐기/땡처리 결정"],
        ["단종품목_재고현황", "관리팀", "월 1회", "메모만", "브랜드는 취급 중인데 이 품목만 단종 처리됐고 재고 남은 것 — 폐기/땡처리 결정"],
        ["RAW_품목마스터", "담당자", "새 브랜드 등장 시만", "직접 수정", "품목 ↔ 브랜드 ↔ 조달유형 ↔ 리드타임 (같은 브랜드 신상품은 자동 등록됨, '미분류'만 확인)"],
        ["RAW_일별재고이력", "—", "—", "수정 금지", "모든 판정이 계산되는 곳"],
    ...
    for term, desc in [
        ...
        ("조달유형", "자체제작 / 국내사입 / 국내위탁 / 해외수입. 리드타임이 다르므로 위험 판정 기준도 다릅니다"),
    ...
    for b in [
        "회색으로 표시된 탭(RAW_일별재고이력·RAW 2개)은 스크립트가 매일 다시 씁니다. 직접 고쳐도 다음날 사라집니다.",
        "나머지 탭에서도 값은 자동으로 채워집니다. 사람이 적는 곳은 메모 컬럼뿐입니다.",
        "RAW_품목마스터는 반대로 사람이 쓰는 탭입니다. 여기 적은 내용은 지워지지 않습니다.",
...
TAB_ORDER = [
    ...
    "악성재고", "악성품절", "샘플의심재고", "구브랜드_방치재고", "단종품목_재고현황", "기타창고_재고현황", "창고목록",
    "RAW_일별재고이력", "RAW_품목마스터",
    "RAW_재고현황", "RAW_판매현황",
]
