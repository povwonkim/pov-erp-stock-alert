...
# 품목마스터 탭 이름. 2026-08-07 '품목마스터' → 'RAW_품목마스터'로 변경.
# 이 탭은 사람이 손으로 만드는 마스터가 아니라 이카운트 원본을 미러링한 것이라 RAW_ 접두어가
# 맞다. 탭 이름이 코드 6곳에 하드코딩돼 있어서 시트에서 이름만 바꾸면 배치가 통째로 죽던 것을
# 상수로 모았다 — 앞으로 이름을 바꿀 땐 여기 한 줄만 고치면 된다.
ITEM_MASTER_TAB = "RAW_품목마스터"
...
def _readme_content() -> list[tuple[str, list[str]]]:
    ...
    for r in [
        ...
        [ITEM_MASTER_TAB, "담당자", "새 브랜드 등장 시만", "직접 수정", "품목 ↔ 브랜드 ↔ 조달유형 ↔ 리드타임 (같은 브랜드 신상품은 자동 등록됨, '미분류'만 확인)"],
...
TAB_ORDER = [
    ...
    "악성재고", "악성품절", "샘플의심재고", "일별재고이력", ITEM_MASTER_TAB,
    "RAW_재고현황", "RAW_판매현황",
]
