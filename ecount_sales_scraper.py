...
def parse_table_rows(header_cells: list[str], body_rows: list[list[str]]) -> list[dict]:
    ...
    for row in body_rows:
        # 이카운트 웹 표는 브랜드/그룹별 "소계" 행을 중간중간 끼워 넣는데, 그 행은 라벨 칸이
        # colspan으로 여러 칸을 병합해서 실제 <td> 개수가 정상 품목 행보다 적다(2026-08-06
        # 발견 — RAW_품목마스터에 품목코드/품명/브랜드가 전부 숫자로 잡힌 "미분류" 유령 행이
        # 계속 생기던 원인). 헤더보다 칸 수가 적은 행은 소계 행으로 보고 건너뛴다 — 안
        # 걸러내면 소계 숫자가 고정 인덱스 매핑 때문에 품목코드/품명/브랜드 자리에 잘못 들어간다.
